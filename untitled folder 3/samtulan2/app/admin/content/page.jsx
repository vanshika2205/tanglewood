                      ...
                      return (
                        ...
                        <div style={{ display:"grid",gridTemplateColumns:"2.5fr 1fr 1fr 1fr 2fr",padding:"16px 20px",borderBottom:"1px solid var(--border-color)",alignItems:"center", background:isOpen ? "var(--glass-bg)" : "transparent" }}>
                          ...
                          <div style={{ display:"flex",gap:8, flexDirection:"column", alignItems:"flex-start" }}>
                            ...
                            <button onClick={()=>setViewLogId(isOpen?null:v._id)} style={{ background:"transparent",color:"var(--text-color)",opacity:0.6,border:"1px solid var(--border-color)",borderRadius:8,padding:"5px 12px",fontSize:11,cursor:"pointer", fontWeight:600, display:"flex", alignItems:"center", gap:6 }}>
                                {isOpen ? "CLOSE REPORT" : "VIEW DETAILED AUDIT"}
                            </button>
                          </div>
                        </div>
                        {isOpen && (
                          <div style={{ padding:"24px 30px", background:"var(--bg-color)", borderBottom:"1px solid var(--border-color)", fontSize:12, color:"var(--text-color)", display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:30 }}>
                            <div style={{ borderLeft:"2px solid #4A9EDB", paddingLeft:16 }}>
                                <div style={{ color:"#4A9EDB", fontWeight:700, fontSize:11, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:12 }}>AI AUDIT RESULT</div>
                                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                                    <div><span style={{opacity:0.6}}>Verdict:</span> <span style={{color:"var(--text-color)", fontWeight:600}}>{v.ai?.verdict?.toUpperCase() || "N/A"}</span></div>
                                    <div><span style={{opacity:0.6}}>Confidence:</span> <span style={{color:"var(--text-color)", fontWeight:600}}>{Math.round((v.ai?.confidence||0))}%</span></div>
                                    <div><span style={{opacity:0.6}}>Risk Flags:</span> <span style={{color:"var(--accent-color)"}}>{v.ai?.reasons?.join(", ") || "None Detected"}</span></div>
                                    <div style={{fontSize:10, opacity:0.4, marginTop:4}}>SCAN ID: {v._id?.slice(-8).toUpperCase()}</div>
                                </div>
                            </div>
                            <div style={{ borderLeft:`2px solid ${v.human?.verdict==="rejected"?"#E55555":"var(--accent-color)"}`, paddingLeft:16 }}>
                                <div style={{ color:"var(--accent-color)", fontWeight:700, fontSize:11, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:12 }}>HUMAN MODERATION</div>
                                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                                    <div><span style={{opacity:0.6}}>Final Status:</span> <span style={{color:"var(--text-color)", fontWeight:600}}>{v.human?.verdict?.toUpperCase() || "PENDING REVIEW"}</span></div>
                                    <div><span style={{opacity:0.6}}>Mod Note:</span> <span style={{color:"var(--text-color)", fontWeight:600, fontStyle:"italic"}}>{v.human?.note || "No notes provided"}</span></div>
                                    <div><span style={{opacity:0.6}}>Processed:</span> <span style={{color:"var(--text-color)"}}>{v.human?.checkedAt ? new Date(v.human?.checkedAt).toLocaleString() : "Awaiting Human"}</span></div>
                                </div>
                            </div>
                            <div style={{ borderLeft:"2px solid var(--border-color)", paddingLeft:16 }}>
                                <div style={{ color:"var(--text-color)", fontWeight:700, fontSize:11, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:12 }}>ENGAGEMENT METRICS</div>
                                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                                    <div><span style={{opacity:0.6}}>Total Views:</span> <span style={{color:"var(--text-color)", fontWeight:600}}>{v.views?.toLocaleString() || 0}</span></div>
                                    <div><span style={{opacity:0.6}}>Total Likes:</span> <span style={{color:"var(--text-color)", fontWeight:600}}>{v.likes?.toLocaleString() || 0}</span></div>
                                    <div><span style={{opacity:0.6}}>Engagement:</span> <span style={{color:"var(--text-color)", fontWeight:600}}>{v.comments?.length || 0} Comments</span></div>
                                </div>
                            </div>
                          </div>
                        )}
