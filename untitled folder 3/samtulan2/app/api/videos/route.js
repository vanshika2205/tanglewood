  ...
  try {
    ...
    if (id) {
      ...
      return Response.json(
        { videos: video ? [video] : [], total: video ? 1 : 0, page: 1, totalPages: 1 },
  ...
  try {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        result = { verdict:"flagged", confidence:50, scores:{}, reasons:["AI scan failed network/fetch"], suggestion:null };
      }
      
      const nextStatus = result.verdict==="rejected"?"rejected": result.verdict==="flagged"?"flagged":"human_review";
      await Video.findByIdAndUpdate(video._id, {
        status: nextStatus,
        ai: { verdict:result.verdict, confidence:result.confidence, scores:result.scores, reasons:result.reasons||[], suggestion:result.suggestion, checkedAt:new Date() },
      });
    }, 100);
